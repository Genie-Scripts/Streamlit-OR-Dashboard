...
class DashboardPage:
    ...
    # --- ▼ここから削除▼ ---
    # _render_period_selector と _calculate_period_dates メソッドは共通コンポーネントに移動したため削除
    # --- ▲ここまで削除▲ ---
    ...
    def _render_kpi_section_with_data(period_df: pd.DataFrame,
    ...
    @staticmethod
    def _calculate_period_kpi(df: pd.DataFrame, start_date: pd.Timestamp,
                             end_date: pd.Timestamp) -> Dict[str, Any]:
        """選択期間のKPIを計算"""
        # (このメソッドは引数がperiod_dfになった以外は変更なし)
        try:
            if df.empty:
                return {}
    ...
    @staticmethod
    def _calculate_or_utilization(df: pd.DataFrame, start_date: pd.Timestamp,
                                 end_date: pd.Timestamp, weekdays: int) -> Tuple[float, int, int]:
        # (このメソッドは変更なし)
        """手術室稼働率を時間ベースで計算"""
        try:
            # 平日のデータのみ
    ...
    def _render_performance_dashboard_with_data(period_df: pd.DataFrame, target_dict: Dict[str, Any],
    ...
    def _render_achievement_summary(period_df: pd.DataFrame, target_dict: Dict[str, Any],
                                  start_date: pd.Timestamp,
