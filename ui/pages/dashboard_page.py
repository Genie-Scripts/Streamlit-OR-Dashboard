...
class DashboardPage:
    ...
    def _calculate_period_kpi(df: pd.DataFrame, start_date: Optional[pd.Timestamp], 
        ...
        try:
            ...
            if not gas_df.empty and 'is_weekday' in gas_df.columns:
                ...
                weekday_gas_cases = gas_cases  # フォールバック
    ...
    def _calculate_or_utilization(df: pd.DataFrame, start_date: Optional[pd.Timestamp], 
        ...
        try:
            ...
            # 手術室フィルタリング：OP-1〜OP12（OP-11A、OP-11Bを除く）11室
            ...
            if or_column:
                # OP-1〜OP12でOP-11A、OP-11Bを除く
                valid_or_pattern = r'^OP-([1-9]|1[0-2])
...
def render():
    """ページルーター用のレンダー関数"""
    DashboardPage.render()  # OP-1からOP-12
                exclude_pattern = r'^OP-11[AB]
    ...
    def _display_period_kpi_metrics(kpi_data: Dict[str, Any], 
        ...
        with col4:
            utilization = kpi_data.get('utilization_rate', 0)
            daily_operating_avg = kpi_data.get('daily_operating_avg', 0)
            
            st.metric(
                "🏥 手術室稼働率",
                f"{utilization:.1f}%",
                delta=f"({daily_operating_avg:.1f}件/日)",
                help="平日（9:00〜17:15）の全手術稼働率"
            )
        ...
        if start_date and end_date:
            period_days = kpi_data.get('period_days', 0)
            weekdays = kpi_data.get('weekdays', 0)
            weekday_operating_cases = kpi_data.get('weekday_operating_cases', 0)
            
            st.caption(
                f"📅 分析期間: {start_date.strftime('%Y/%m/%d')} ～ {end_date.strftime('%Y/%m/%d')} "
                f"({period_days}日間, 平日{weekdays}日) | "
                f"平日稼働時間手術: {weekday_operating_cases}件"
            )
...
def render():
    """ページルーター用のレンダー関数"""
    DashboardPage.render()  # OP-11A、OP-11B
                
                weekday_df['is_valid_or'] = (
                    weekday_df[or_column].astype(str).str.match(valid_or_pattern, na=False) &
                    ~weekday_df[or_column].astype(str).str.match(exclude_pattern, na=False)
                )
                
                or_filtered_df = weekday_df[weekday_df['is_valid_or'] == True]
            else:
